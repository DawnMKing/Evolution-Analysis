%% modify_data.m *******************************************************
% This may be used to move specific data to other places. The template is that of main_babies.m and
...
ddm = 0.01;    death_max = [0.01]; %percent of random babies dying varies from 0 to this value.
...
for mu = mutability
  ...
  SIMOPTS = struct('source',source,'SIMS',SIMS,'NGEN',NGEN,'IPOP',IPOP,'limit',limit,...
  ...
  make_dir = 1; [base_name,dir_name] = NameAndCD(make_dir);
  for run = SIMS
    ...
    %move_data(base_name,g,run_name);
...
%     end
    if exist(['trace_cluster_seed_' g run_name '.mat'])==2
